...
let fireworksInterval; // Variable to store the interval ID

function fireworks() {
    const duration = 15 * 1000,
        animationEnd = Date.now() + duration,
        defaults = { count: 300, startVelocity: 30, spread: 360, scalar: 1.2, ticks: 60, zIndex: 99 };
    ...
    fireworksInterval = setInterval(function () {
        ...
        if (timeLeft <= 0) {
            clearInterval(fireworksInterval); // Clear the interval when time is up
            return;
        }
...
function showCompliment(type) {
    const compliment = document.getElementById('compliment_modal');
    const comp = document.getElementById('compliment');
    const comptext = document.getElementById('compliment-text');
    const finishtext = document.getElementById('finish-text');
    const closeBtn = document.getElementById('close-button');

    compliment.style.display = "grid";
    closeBtn.style.display = "block";

    // Images
    const boygo = document.getElementById('boygo');
    const boytuwa = document.getElementById('boytuwa');
    const boymagnify = document.getElementById('boymagnify');
    const boysad = document.getElementById('boysad');
    const boy = document.getElementById('boy');
    const check = document.getElementById('check');
    const cross = document.getElementById('cross');
    const star = document.getElementById('star');
    const button = document.getElementById('button-compliment');
    const resultchoices = document.getElementById('result-button-choices');

    if (type == 'success') {
        let compliments = ["Great Job!", "Awesome!", "Well Done!", "Nice!"];
        shuffleArray(compliments);
        comp.innerHTML = compliments[0];
        ...
        boygo.style.display = "block";
        boytuwa.style.display = "block";
        boymagnify.style.display = "none";
        boysad.style.display = "none";
        ...
        let compliments = ["Nice Try!", "Too Bad!", "Almost!", "It's Okay!"];
        shuffleArray(compliments);
        comp.innerHTML = compliments[0];
        ...
        boygo.style.display = "none";
        boytuwa.style.display = "none";
        boymagnify.style.display = "block";
        boysad.style.display = "block";
        boy.style.display = "none";

        button.innerHTML = "Keep it up!";
        resultchoices.style.display = "none";
    } else if (type == 'finish') {
